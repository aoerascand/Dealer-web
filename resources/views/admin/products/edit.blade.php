...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-lg border border-slate-100 p-8">
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-5">
            <label class="block text-sm font-semibold text-slate-700 mb-2">Gambar Motor (Biarkan kosong jika tidak diubah)</label>
            @if($product->gambar)
                <img src="{{ asset('storage/' . $product->gambar) }}" class="w-32 h-24 object-cover mb-2 rounded shadow-sm">
            @endif
